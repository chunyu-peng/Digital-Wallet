...
#define BLOCK_FILE_PATH "./block1.txt"  // read block file
...
// send result size and transactions
struct QueryResult {
    int size;
    Transaction transaction_list[LIS_MAX_SIZE];
};

// list of all transactions in block1
...
// read block file and add it to list of transactions
void parse_block_file() {
    ...
    while (!input_file.eof()) {
        ...
        t.serial_number = atoi(serial_number_str.c_str());
        ...
        t.amount = atoi(amount_str.c_str());
...
// append new transactions to the end of file
void log_new_transaction(Transaction t) {
    transaction_list.push_back(t);

    fstream output_file;
    output_file.open(BLOCK_FILE_PATH, ios_base::app | ios_base::in | ios::binary);
...
// look for transactions that either has username as sender or receiver
...
void talk_to_serverM() {
    stringstream ss;
    // store largeret serial number
    ss << max_serial << " ";
    ss << query_result.size << " ";
    for (int i = 0; i < query_result.size; i++) {
        ss << query_result.transaction_list[i].serial_number << " " 
           << query_result.transaction_list[i].sender << " " 
           << query_result.transaction_list[i].receiver << " " 
           << query_result.transaction_list[i].amount << " ";
    }

    int sendto_result = sendto(slave_server, ss.str().c_str(),ss.str().length(), FLAG, p->ai_addr, p->ai_addrlen);
    if (sendto_result == -1) {
...
void handle_query() {
    ...
    iss >> query.serial_number;

    // again, choose operation based on interger assigned to serial number
